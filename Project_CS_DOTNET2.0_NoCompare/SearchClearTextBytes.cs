...
namespace Project_CS
{
    ...
    public class SearchClearTextBytes
    {
        /// デバッグ出力の可否
        ...
        /// ハッシュ処理用クラスのインスタンス
        private ComputeHashToBytes computeHash = new ComputeHashToBytes();
        ...
        public SearchClearTextBytes(int alg_index, string targetStr, int strLen, int threadMax, int mode, Boolean use_multiThread, Boolean use_debug)
        {
        ...
        public string Get_ClearText(int threadMax)
        {
            ...
            // 平文が""かどうかを判定する。
            //-------------------------------------------------------------------------//
            computeHash.ComputeHash_Common(Algorithm_Index, Encoding.ASCII.GetBytes(""));
            /*
        #if USING_LINQ
        ...
        protected bool Get_NextClearText_Group_All(int threadNum, int target_strLength)
        {
            ...
            for (int index = chrStart[selectIndex][threadNum]; index < chrEnd[selectIndex][threadNum]; index++)
            {
                ...
                // 指定したアルゴリズムにてハッシュ値を生成する。
                computeHash.ComputeHash_Common(Algorithm_Index, srcStr[threadNum]);
                /*
            #if USING_LINQ
