...
import (
	...
	computehashToString "./computeHashToString"
)
...
func GetClearText(threadMax int) (string, int) {
	...
	//-------------------------------------------------------------------------//
	workStr := ""
	if targetHashedStr == computehashToString.ComputeHashCommon(AlgorithmIndex, *(*[]byte)(unsafe.Pointer(&workStr))) {
		return "", 0
	}
...
func GetNextClearTextGroupAll(threadNum int, targetStrLength int) bool {
	...
	for index := chrStart[selectIndex][threadNum]; index < chrEnd[selectIndex][threadNum]; index++ {
		...
		// 指定したアルゴリズムにてハッシュ値を生成する。
		if targetHashedStr == computehashToString.ComputeHashCommon(AlgorithmIndex, srcStr[threadNum]) {
			return true
		}
	}
