...
public class SearchClearTextString {
    /// 処理済み平文の出力用文字列（デバッグ用）
    ...
    /// ハッシュ処理用クラスのインスタンス
    private ComputeHashToString computeHash = new ComputeHashToString();

    /// 各スレッド処理終了時結果文字列
    private String[] resultStr;
    private String targetHashedStr;
    ...
    public SearchClearTextString(int alg_index, String targetStr, int strLen, int threadMax, int mode, boolean arg_enableMultiThread, boolean enableDebug) {
    ...
    public String Get_ClearText(int threadMax) {
        ...
        // 平文が""かどうかを判定する。
        //-------------------------------------------------------------------------//
        if (targetHashedStr.equals(computeHash.ComputeHash_Common(Algorithm_Index, "".getBytes()))) {
            return "";
        }
    ...
    protected boolean Get_NextClearText_Group_All(int threadNum, int target_strLength) {
        ...
        for (int index = chrStart[selectIndex][threadNum]; index < chrEnd[selectIndex][threadNum]; index++) {
            ...
            // 指定したアルゴリズムにてハッシュ値を生成する。
            if (targetHashedStr.equals(computeHash.ComputeHash_Common(Algorithm_Index, srcStr[threadNum]))) {
                return (true);
            }
        }
