...
public class MySolution {
    ...
    public void Main(String open_FileName, int thread_count, int search_max_length, int search_mode, boolean enableMultiThread, boolean use_debug) {
        ...
        */
        ...
        System.out.println("Collation type     : byte[]");
        if (enableMultiThread) {
            System.out.println("thread count       : " + Integer.toString(thread_count));
        } else {
            System.out.println("multiThread        : " + Boolean.toString(enableMultiThread));
        }
        ...
        // 総当たり検索実行
        search(target_hashed_text, algorithm, thread_count, ClearTextMaxLength, search_mode, enableMultiThread, use_debug);

        long end = System.currentTimeMillis();
        System.out.println("Total Execute time ... " + (end - start)/1000.0  + " [s]\n");
    }
    ...
    private void search(String target_hashed_text, String algorithm, int threadMax, int searchClearText_MaxLength, int search_mode, boolean enableMultiThread, boolean use_debug) {
        // 使用するスレッド数の指定チェック
        if ((threadMax != 1)
        && (threadMax != 2)
        && (threadMax != 4)
        ...
        // 結果の初期化
        String resultStr = "";
        SearchClearTextBytes searchClearText;
        ...
        for (int target_strLen = 1; target_strLen <= searchClearText_MaxLength; target_strLen++) {
            ...
            // 平文検索処理用インスタンスの生成
            searchClearText = new SearchClearTextBytes(Algorithm_Index, target_hashed_text, target_strLen, threadMax, 0, enableMultiThread, use_debug);
            ...
            if (resultStr != null) {
                System.out.println("元の文字列が見つかりました！\r\n"
                                ...
                                + "結果 = " + resultStr + "\r\n"
                                + "\r\n"
                                + "解析時間 = " + timeformatter.format(ts) + " ( " + ts/1000.0  + " [s] )\n");
                break;
